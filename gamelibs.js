...
function sdRoundCone(p, a, b, r1, r2){
    ...
    let y = dot(pa, ba);
    let z = y-l2;
    let temp = minus(times(pa, l2), times(ba,y));
    let x2 = dot(temp,temp);
    let y2 = y*y*l2;
    let z2 = z*z*l2;
...
function cast(x, y, de){
    let dir = normalize([x*0.35, y*0.35, 0.5515]);
    
    dir = rotX(dir, -camAngle[1]);
    dir = rotY(dir, camAngle[0]);

    return march(de, camPos, dir);
}

let displayWidth = 0.6;
function normalizeScreenCoords(x,y){
    return [2*((x - (window.innerWidth*displayWidth-window.innerHeight)/2) / window.innerHeight) - 1, 2*(1-(y / window.innerHeight)) - 1];
}
